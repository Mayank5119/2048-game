...
class GAME(tk.Frame):
            ...
            for j in range(4):
                ...
                if cell_value == 0:
                    ...
                    self.cells[i][j]["number"].configure(bg=c.CELL_COLORS[cell_value],
                                                         ...
                                                         font=c.CELL_NUMBER_FONTS[cell_value], text=str(cell_value))
    ...
    def game_over(self):
        if any(2048 in row for row in self.matrix):
            game_over_frame = tk.Frame(self.main_grid, borderwidth=2).place(relx=0.5, rely=0.5, anchor="center")
            tk.Label(game_over_frame, text="YOU WIN!", bg=c.WINNER_BG, fg=c.GAME_OVER_FONT_COLOR,
                     font=c.GAME_OVER_FONT_COLOR).grid(row=0, column=0, padx=5, pady=5)
        elif not any(0 in row for row in
                     self.matrix) and not self.horizontal_move_exists() and not self.vertical_move_exists():
            game_over_frame = tk.Frame(self.main_grid, borderwidth=2).place(relx=0.5, rely=0.5, anchor="center")
            tk.Label(game_over_frame, text="YOU LOSE!", bg=c.LOSER_BG, fg=c.GAME_OVER_FONT_COLOR,
                     font=c.GAME_OVER_FONT_COLOR).grid(row=0, column=0, padx=5, pady=5)
